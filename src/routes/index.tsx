...
function Hero() {
	return (
		<section
			...
			style={{
				background:
					"radial-gradient(ellipse 70% 55% at 8% 18%, rgba(79,70,229,0.12) 0%, transparent 60%), radial-gradient(ellipse 55% 45% at 88% 78%, rgba(6,182,212,0.09) 0%, transparent 55%)",
			}}
		>
			<div
			...
			<div className="page-wrap relative">
				<div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
					<div>
						<h1
							className="display-title rise-in mb-6 text-5xl font-bold leading-[1.08] tracking-tight text-foreground md:text-6xl"
							style={{ animationDelay: "70ms" }}
						>
							Every tool you need to make{" "}
							<span
								style={{
									background:
						...
						<p
							className="rise-in mb-8 max-w-lg text-lg leading-relaxed text-muted-foreground"
							style={{ animationDelay: "140ms" }}
						>
							Salary calculators, tax comparisons, career tools, and retirement
							planners — all free, all private, all in one place.
						</p>
						<div
						...
						>
							<a
								href="#tools"
								className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-6 py-3 text-base font-semibold text-white shadow-lg shadow-primary/20 transition-all duration-180 hover:opacity-90 hover:-translate-y-0.5"
							>
								Explore all tools
							...
							<a
								href="#why"
								className="inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-background px-6 py-3 text-base font-semibold text-foreground transition-colors duration-180 hover:bg-secondary"
							>
								Why it's free
							</a>
						</div>
						<div
							className="rise-in mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted-foreground"
							style={{ animationDelay: "280ms" }}
						>
							{["100% Free", "No sign-up", "Zero tracking"].map((t) => (
					...
					<div
						className="rise-in lg:flex lg:justify-end"
...
function ToolChipsPreview() {
	return (
		<div className="w-full max-w-sm lg:max-w-xs xl:max-w-sm">
			<p className="island-kicker mb-4">8 tools and counting</p>
			<div className="grid grid-cols-2 gap-3">
				{TOOLS.slice(0, 6).map((tool, i) => (
					<div
						key={tool.id}
						className="rise-in island-shell flex items-center gap-2.5 rounded-xl p-3"
						style={{ animationDelay: `${240 + i * 55}ms` }}
					>
						<div
							className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${tool.bg}`}
						>
							<tool.icon
								className={`h-4 w-4 ${tool.status === "live" ? tool.color : "text-muted-foreground"}`}
							/>
						</div>
...
function ToolCard({ tool }: { tool: (typeof TOOLS)[number] }) {
	...
	return isLive ? (
		<a href={tool.href ?? "#"} className="h-full">
			{inner}
		</a>
	) : (
